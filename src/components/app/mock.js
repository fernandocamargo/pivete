/* eslint import/no-anonymous-default-export: [2, {"allowObject": true}] */
export default {
  columns: [
    {
      content: "2021",
      type: "year",
      details: [
        {
          content: "Jan",
          type: "month",
          details: [
            { content: "Actual", type: "itemtype" },
            { content: "Budget", type: "itemtype" },
            { content: "Actual vs. Budget", type: "itemtype" },
          ],
        },
        {
          content: "Fev",
          type: "month",
          details: [
            { content: "Actual", type: "itemtype" },
            { content: "Budget", type: "itemtype" },
            { content: "Actual vs. Budget", type: "itemtype" },
          ],
        },
      ],
    },
  ],
  rows: [
    {
      content: "Office 1",
      type: "office_id",
      value: [
        {
          content: 1,
          type: "year",
          value: [
            {
              content: 2,
              type: "month",
              value: [
                { content: 3, type: "itemtype" },
                { content: 4, type: "itemtype" },
                { content: "5%", type: "itemtype" },
              ],
            },
            {
              content: 6,
              type: "month",
              value: [
                { content: 7, type: "itemtype" },
                { content: 8, type: "itemtype" },
                { content: "9%", type: "itemtype" },
              ],
            },
          ],
        },
      ],
      details: [
        {
          content: "Finance, BI & Operations",
          type: "cc_level1",
          value: [
            {
              content: 10,
              type: "year",
              value: [
                {
                  content: 11,
                  type: "month",
                  value: [
                    { content: 12, type: "itemtype" },
                    { content: 13, type: "itemtype" },
                    { content: "14%", type: "itemtype" },
                  ],
                },
                {
                  content: 15,
                  type: "month",
                  value: [
                    { content: 16, type: "itemtype" },
                    { content: 17, type: "itemtype" },
                    { content: "18%", type: "itemtype" },
                  ],
                },
              ],
            },
          ],
        },
      ],
    },
    {
      content: "Office 2",
      type: "office_id",
      value: [
        {
          content: 19,
          type: "year",
          value: [
            {
              content: 20,
              type: "month",
              value: [
                { content: 21, type: "itemtype" },
                { content: 22, type: "itemtype" },
                { content: "23%", type: "itemtype" },
              ],
            },
            {
              content: 24,
              type: "month",
              value: [
                { content: 25, type: "itemtype" },
                { content: 26, type: "itemtype" },
                { content: "27%", type: "itemtype" },
              ],
            },
          ],
        },
      ],
      details: [
        {
          content: "Finance, BI & Operations",
          type: "cc_level1",
          value: [
            {
              content: 28,
              type: "year",
              value: [
                {
                  content: 29,
                  type: "month",
                  value: [
                    { content: 30, type: "itemtype" },
                    { content: 31, type: "itemtype" },
                    { content: "32%", type: "itemtype" },
                  ],
                },
                {
                  content: 33,
                  type: "month",
                  value: [
                    { content: 34, type: "itemtype" },
                    { content: 35, type: "itemtype" },
                    { content: "36%", type: "itemtype" },
                  ],
                },
              ],
            },
          ],
        },
        {
          content: "Marketing",
          type: "cc_level1",
          value: [
            {
              content: 37,
              type: "year",
              value: [
                {
                  content: 38,
                  type: "month",
                  value: [
                    { content: 39, type: "itemtype" },
                    { content: 40, type: "itemtype" },
                    { content: "41%", type: "itemtype" },
                  ],
                },
                {
                  content: 42,
                  type: "month",
                  value: [
                    { content: 43, type: "itemtype" },
                    { content: 44, type: "itemtype" },
                    { content: "45%", type: "itemtype" },
                  ],
                },
              ],
            },
          ],
        },
      ],
    },
  ],
};
